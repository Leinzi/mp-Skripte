...
function fetchAndBuildActivityFeed() {
  ...
  const activityFeedURL = `https://www.moviepilot.de/api/homepage/dashboard/${stream}?per_page=${PER_PAGE}&page=${currentPage}`
...
// Elements
function activityContainerElement(activity, type) {
  ...
  if (type == 'followship') {
    activityMetaHTML= activityMetaForFollowship(activity)
    rating = { verbalization: 'Vorgemerkt' }
  } else if (type == 'rating') {
    activityMetaHTML = activityMetaForRating(activity)
  } else if (type == 'comment') {
    activityMetaHTML = activityMetaForComment(activity)
  }
...
function activityMetaForFollowship(followship) {
  return `
    <span class="activity--meta">
      merkte sich
      <a href="${followship.url}" movie-title>${followship.title}</a>
      ${parseMetaShort(followship)}
      vor
    </span>
  `
}

function activityMetaForRating(rating) {
  return `
    <span class="activity--meta">
      bewertete
      <a href="${rating.url}" movie-title>${rating.title}</a>
      ${parseMetaShort(rating)}
    </span>
  `
}

function activityMetaForComment(comment) {
  return `
    <span class="activity--meta">
      kommentierte
      <a href="${comment.url}" movie-title>${comment.title}</a>
    </span>
  `
}
